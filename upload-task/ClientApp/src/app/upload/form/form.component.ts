...
import {FileInf} from "../../_interfaces/file.model";
...
export class FormComponent implements OnInit {
  public file_inf: FileInf | undefined;
  public img: any;
  public imgURL: any;
  public message: string = '';
  frm_upload: FormGroup | undefined;
  file: any;
  ...
  preview(files: string | any) {
    ...
    this.file_inf = {
      ...
      FileSize    : this.cm.convertFileSize(1, files[0].size),
      FilePathSrc : this.cm.convertFileSize(2, files[0].size),
      FilePathDest: this.cm.convertFileSize(0, files[0].size),
      UploadDate  : new Date(),
    }
  }
  ...
  saveUpload() {
    if (this.file == null) {
      this.cm.alerta(this.cm.ERROR, "Please Select a file !")
      return
    } else {
      this.prog_bar(1)
      const form_data = new FormData();
      form_data.append('files', this.file);
      this.upload_service.saveUpload(form_data).subscribe(res => {
          if (res.status == 1) {
            this.file_inf = {
              FileName    : "",
              FileMime    : "",
          ...
          this.cm.alerta(res.status, res.message)
          this.prog_bar(0)
        }
      )
    }
  ...
  prog_bar(type: number) {
    this.upload_progress = type;
  }
}
